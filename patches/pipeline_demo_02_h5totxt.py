...
import pose2Dto3D
import pose3D 
# PFE-SOUFIAN patch: TensorFlow stub. TF is only used for
# tf.keras.backend.clear_session() (GPU memory cleanup) on line 102; the actual
# 2D->3D math is in pure-numpy pose2D / pose2Dto3D / pose3D modules. Avoiding
# TF keeps the container small and matches our PyTorch-only stack.
class _StubTF:
    class keras:
        class backend:
            @staticmethod
